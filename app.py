...
st.write("Hello! This is a small project that I've visually represented in this web application. It might seem a bit chaotic, so let me explain it: In the ground, there are different layers called facies, and each has its unique properties. When we drill a hole in the ground, we obtain samples at various depths, each with distinct properties. By knowing the actual layers in the ground in one region and extracting samples, we can theoretically predict the geomorphology and lithographic history in another nearby region where we don't know the facies but can take samples. This web app showcases a significant part of the project I've been working on and demonstrates some of the decisions I've made through various graphs. If you're interested in learning more, I've included an amateur paper at the bottom of this page. The data for this project is sourced from a block in the Dutch sector of the North Sea. Our initial step in creating an AI model involves cleaning and preparing this data.")

st.write("Once the data was processed and labeled, we separated the training features from the target variable. The initial model employed linear regression and delivered varying accuracy levels based on different parameter combinations. Specifically, we explored parameters such as density, porosity, P-wave impedance, relative P-wave impedance, and gamma-ray. These parameter choices emerged from experimentation and fine-tuning until we identified the combination that yielded the highest accuracy. You have the opportunity to experiment with these parameters and observe how predictions change based on the factors you select.")
